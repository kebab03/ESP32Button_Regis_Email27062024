...
class ToggleScreen(Screen):
    ...
    def update_buttons(self, num_buttons, button_labels):
        self.num_buttons = num_buttons
        self.button_labels = button_labels
        self.button_states = ['off'] * num_buttons
        print(" 126  button_states ")
        self.update_layout()
    ...
    def submit_form(self, instance):
        ...
        print("351 response ",response)
        print(response.json()['button_labels'])
        if response.status_code == 200:
            print(response.json()['num_buttons'])
            # Richiesta GET riuscita
            print(" 143 ")
            self.manager.get_screen('toggle').update_buttons( response.json()['num_buttons'],response.json()['button_labels'])
