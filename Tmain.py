...
class WWToggleScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Header or fixed layout
        float_layout = ColoredBoxLayout(size_hint_y=None, height=150, background_color=(0.55, 1, 0.2, 1))
        ...
        add_button = Button(
            ...
            size=(dp(150), dp(60)),
            background_color=(1, 1, 0, 0.8), color=(0, 0, 0, 1)
        )
    ...
    def Aupdate_buttons(self, num_buttons, button_labels, button_states):
        ...
        self.button_states = button_states
        print(" 644  linee ")
        print(" line 645 num_buttons ", num_buttons )
        print(" line 646 button_labels ", button_labels)
    #self.button_layout.clear_widgets() # Elimina sono la parte di sotto del screen 
        #self.main_layout.clear_widgets() #Elimina  intero  screen 
        self.update_layout()
    ...
    def submit_form(self, instance):       
        ...
        if response.status_code == 200:
            ...
            #    self.manager.get_screen('toggle').update_buttons(num_buttons, button_labels, button_states)
        ...
        for i in range(self.num_buttons):
            ...
            if i < len(predefined_colors):
                ...
                background_color = (0, 1 / (i + 2), 1 / (i + 1), 1)
                    ...
                    # Remove the ScrollView from its current parent
    ...
    #     self.grid_layout.remove_widget(button)
    #     url = "http://localhost:5000/remove"
    #     data = {"buttonLabel": button.text}
    #     headers = {'User-Agent': 'MyKivyApp'}
    #     login_screen = self.manager.get_screen('login')
    #     response = login_screen.session.post(url, json=data, headers=headers)
    #     if response.status_code == 200:
    ...
    def delete_button(self, horizontal_layout, button, label, data_label, submiton):
        ...
        print(" 748 Toggle.text name to button:", button.text)
        print("label name to remove:", label.text)
        print("data_label.text name to remove:", data_label.text)
        toggle_name = data_label.text  # Use the passed label argument
        
        login_screen = self.manager.get_screen('login')
        
        print(" 755 Toggle_name to remove:---", toggle_name)
        url = "http://localhost:5000/remove"
        data = {"buttonLabel": toggle_name}
        headers = {'User-Agent': 'MyKivyApp'}
        
        print(" 760  Sending request to remove toggle")
        response = login_screen.session.post(url, json=data, headers=headers)
